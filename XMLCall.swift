...
class XMLCall : Call {
  ...
  override func performCall(ctx: CallContext, index: Int) {
    ...
    if (ctx.actives.count < ctx.dancers.count) {
      ...
      //  Now add the paths
      ctx3.dancers.enumerate().forEach { (i,d) in
    ...
    xmlmap.enumerate().forEach { (i3,m) in
      let p = Path(allp[m>>1])
  ...
  func computeFormationOffsets(ctx1:CallContext, _ ctx2:CallContext) -> [Vector3D] {
    var dvbest = [Vector3D]()
    ...
    [CGFloat(0),CG_PI/2,CG_PI,CG_PI*3/2].forEach { angle in
      var dv = [Vector3D](count:ctx1.dancers.count, repeatedValue:Vector3D(x:0,y:0))
      var dtot:CGFloat = 0.0
      ctx1.actives.enumerate().forEach { (i,d1) in
        let v1 = d1.location
